...
''' <summary>64bit整数のデータ型を表現するクラスです。</summary>
Public NotInheritable Class LongDataType
    Inherits DataType
    ...
    Public Overrides Function CanTo(input As String) As Boolean
        ...
        Else
            Dim tmp As Long
            Return Long.TryParse(input, tmp)
        End If
    End Function
    ...
    Public Overrides Function ToValue(input As String) As Object
        ...
        Else
            Dim tmp As Long
            If Long.TryParse(input, tmp) Then
                Return tmp
            Else
                Throw New FormatException($"unable to convert ""{input}"" to long")
            End If
        End If
    End Function
