...
''' <summary>倍精度浮動小数のデータ型を表現するクラスです。</summary>
Public NotInheritable Class DoubleDataType
    Inherits DataType
    ...
    Public Overrides Function CanTo(input As String) As Boolean
        ...
        Else
            Dim tmp As Double
            Return Double.TryParse(input, tmp)
        End If
    End Function
    ...
    Public Overrides Function ToValue(input As String) As Object
        ...
        Else
            Dim tmp As Double
            If Double.TryParse(input, tmp) Then
                Return tmp
            Else
                Throw New FormatException($"unable to convert ""{input}"" to double")
            End If
        End If
    End Function
